...
bool _isInTarget(double? max, {double? min, double? targetMin, double? targetMax, bool inclusive = true}) {
  ...
  if ((targetMin != null && min0! <= targetMin) || (targetMax != null && max! >= targetMax)) {
    // Check if target is inclusive, don't show error color in that case
    if (inclusive && (min0 == targetMin || max == targetMax)) {
      return true;
    }
...
class TargetLineDecoration extends DecorationPainter {
  ...
  Offset applyPaintTransform(ChartState state, Size size) {
    final size0 = (state.defaultPadding + state.defaultMargin).deflateSize(size);
    final maxValue = state.data.maxValue - state.data.minValue;
    final scale = size0.height / maxValue;
    final minValue = state.data.minValue * scale;

    return Offset(
      state.defaultMargin.left,
      (size0.height - (lineWidth / 2)) - scale * (target ?? 0) + minValue + state.defaultMargin.top,
    );
  }
  ...
  void draw(Canvas canvas, Size size, ChartState state) {
    ...
    if (dashArray != null) {
      canvas.drawPath(dashPath(path, dashArray: dashArray!), linePaint);
    } else {
...
class TargetAreaDecoration extends DecorationPainter {
  ...
  Size layoutSize(BoxConstraints constraints, ChartState state) {
    ...
    return Rect.fromPoints(
      Offset(0.0, -scale * targetMax + minValue + areaPadding.vertical),
      Offset(constraints.maxWidth, -scale * targetMin + minValue),
    ).size;
  }
  ...
  Offset applyPaintTransform(ChartState state, Size size) {
    ...
    return Offset(
      areaPadding.left,
      size0.height - scale * targetMax + minValue + state.defaultMargin.top + state.defaultPadding.top,
    );
  }
  ...
  void draw(Canvas canvas, Size size, ChartState state) {
    ...
    if (dashArray != null) {
      canvas.drawPath(
        dashPath(rectPath, dashArray: dashArray!),
        Paint()
          ..color = targetLineColor
          ..strokeWidth = lineWidth
          ..style = PaintingStyle.stroke,
