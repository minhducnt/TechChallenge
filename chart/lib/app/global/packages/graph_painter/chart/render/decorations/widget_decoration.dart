...
typedef WidgetDecorationBuilder<T> =
    Widget Function(BuildContext context, ChartState<T> chartState, double itemWidth, double verticalMultiplier);
...
class WidgetDecoration extends DecorationPainter {
  ...
  WidgetDecoration({required this.widgetDecorationBuilder, this.margin = EdgeInsets.zero});
    ...
    return ChartDecorationChildRenderer(
      ...
      LayoutBuilder(
        builder: (context, constraints) {
          ...
          final size = (state.defaultMargin + state.defaultPadding).deflateSize(constraints.biggest);
          ...
          return widgetDecorationBuilder(context, state, itemWidth, verticalMultiplier);
        },
      ),
    );
  }
