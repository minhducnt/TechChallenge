...
class _RenderLeafChartItem<T> extends RenderBox {
  ...
  void handleEvent(PointerEvent event, BoxHitTestEntry entry) {
    ...
    if (event is PointerHoverEvent) {
      _state.behaviour.onItemHoverEnter?.call(ItemBuilderData<T>(item, itemIndex, listIndex));
    }

    if (event is PointerHoverEvent) {
      _state.behaviour.onItemHoverExit?.call(ItemBuilderData<T>(item, itemIndex, listIndex));
    }
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    canvas.translate(itemOffset.dx, itemOffset.dy);

    // Use item painter from ItemOptions to draw the item on the chart
    final itemPainter = _itemOptions.geometryPainter(item, _state.data, _itemOptions, drawDataItem);

    // Draw the item on selected position
    itemPainter.draw(canvas, size, drawDataItem.getPaint(size));
