...
class ChartData<T> {
  ...
  factory ChartData.fromList(List<ChartItem<T>> items, {double? axisMax, double? axisMin, double? valueAxisMaxOver}) {
    return ChartData([items], axisMin: axisMin, axisMax: axisMax, valueAxisMaxOver: valueAxisMaxOver);
  }
  ...
  static double _getMinValue<T>(List<ChartItem<T?>> items, double? valueAxisMin) {
    ...
    return min(valueAxisMin ?? 0.0, minItems.reduce(min));
  }
...
class ChartItemsLerp {
  ...
  static List<List<ChartItem<T?>>> lerpValues<T>(List<List<ChartItem<T?>>> a, List<List<ChartItem<T?>>> b, double t) {
    ...
    final emptyList = <ChartItem<T>>[];

    /// Generate new list fot animation step, add items depending on current [_listLength]
    return List<List<ChartItem<T?>>>.generate(listLength.ceil(), (int index) {
      return _lerpItemList<T>(a.length > index ? a[index] : emptyList, b.length > index ? b[index] : emptyList, t);
    });
  }
  ...
  static List<ChartItem<T?>> _lerpItemList<T>(List<ChartItem<T?>?> a, List<ChartItem<T?>?> b, double t) {
    ...
    /// Empty value for generated list.
    final emptyValue = ChartItem<T?>(0.0, value: null, min: 0.0);

    return List<ChartItem<T?>>.generate(listLength.ceil(), (int index) {
      // If old list and new list have value at [index], then just animate from,
      // old list value to the new value
      final firstItem = index < a.length ? a[index] : null;
      final secondItem = index < b.length ? b[index] : null;

      if (index < a.length && index < b.length) {
        if (secondItem != null && firstItem != null) {
          return secondItem.animateFrom(firstItem, t);
        } else if (secondItem != null) {
          return secondItem.animateFrom(emptyValue, t);
        } else if (firstItem != null) {
          return firstItem.animateTo(emptyValue, t);
        }
      ...
      if (index < b.length) {
        if (secondItem == null || secondItem.isEmpty) {
          return secondItem ?? emptyValue;
        }

        // If item is appearing then it's time to animate is
        // from time it first showed to end of the animation.
        final value = listLength.floor() == index ? ((listLength - listLength.floor()) * t) : t;
        return secondItem.animateFrom(emptyValue, value);
      }
