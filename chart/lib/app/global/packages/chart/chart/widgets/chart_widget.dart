...
class _ChartWidget<T> extends StatelessWidget {
  const _ChartWidget({this.height = 240.0, this.width, required this.state, super.key});
  ...
  double _calcItemWidthNonScrollable() {
    return max(state.itemOptions.minBarWidth ?? 0.0, state.itemOptions.maxBarWidth ?? 0.0);
  }
  ...
  double _calcItemWidth(double frameWidth) {
    ...
    final sizeTween = Tween(begin: _calcItemWidthNonScrollable(), end: _calcItemWidthForScrollable(frameWidth));
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return SingleChildScrollView(
          ...
          child: Container(constraints: BoxConstraints.tight(size), child: ChartRenderer(state)),
        );
      },
    );
  }
